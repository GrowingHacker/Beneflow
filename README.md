...
**当前状态：661 个用例全部通过（Release 约 1 分钟，0 失败、0 警告）。**
...
规模：后端源码约 11.3k 行 / 113 个文件（`src/Beneflow.Api/**/*.cs`，不含 EF 自动生成的 `Migrations/`），测试代码约 12.2k 行 / 50 个文件（`tests/**/*.cs`），比例约 1.08 : 1。597 个测试方法（`[Fact]` 576 + `[Theory]` 21），Theory 展开后共 661 个用例。
